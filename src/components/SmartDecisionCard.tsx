...
const preferences: Array<{ id: DecisionPreference; label: string }> = [
  { id: 'best', label: 'En mantıklısı' },
  { id: 'economic', label: 'En ucuz' },
  { id: 'fast', label: 'En hızlı' },
  { id: 'healthy', label: 'Daha sağlıklı' },
  { id: 'ready', label: 'Hazır gelsin' },
];
  ...
  const options: DecisionOption[] = [
    {
      ...
      timeValue: prepTime,
      helper: `${calories} kcal • evdeki malzeme uyumu %${matchPercent}`,
      icon: 'home-outline',
    },
    {
      mode: 'market',
      ...
      helper: marketTotal > 0
        ? `${commerceMarketDemo.marketName} • sadece eksikler`
        : 'Eksik ürün yok • market maliyeti yok',
      icon: 'basket-outline',
    },
    {
      mode: 'restaurant',
      ...
      helper: bestRestaurant ? `${bestRestaurant.name} • ⭐ ${bestRestaurant.rating.toFixed(1)}` : 'Restoran alternatifi',
  ...
  const recommendationText =
    recommendation.mode === 'home'
      ...
      : recommendation.mode === 'market'
        ...
        : `${recipeTitle} için hazır sipariş daha pahalı olabilir ama zamandan tasarruf sağlar.`;
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        <View style={styles.titleRow}>
          ...
          <View style={styles.headerCopy}>
            ...
            <Text style={styles.title}>Evde yap mı, hazır söyle mi?</Text>
          </View>
        </View>
          ...
          return (
            ...
            >
              <View style={styles.optionTop}>
                ...
                {recommended ? <Text style={styles.recommendedPill}>Önerilen</Text> : null}
              </View>
              <Text style={styles.optionTitle}>{option.title}</Text>
              <Text style={styles.optionPrice}>₺{option.cost.toFixed(0)}</Text>
      ...
      <View style={styles.metricGrid}>
        <View style={styles.metric}>
          <Text style={styles.metricValue}>₺{savings.toFixed(0)}</Text>
        ...
        <View style={styles.metric}>
          ...
          <Text style={styles.metricLabel}>En hızlı</Text>
        </View>
      </View>
      ...
      >
        ...
        <Text style={styles.ctaText}>{recommendation.title} seçeneğini aç</Text>
      </TouchableOpacity>
      <Text style={styles.note}>
        Fiyatlar tahminidir; market ve restoran seçenekleri beta sürecinde netleşir.
      </Text>
    </View>
  );
}
